...
import type { Payment, Supplier, CashBalance } from '@shared/types'
...
export default function KassaMexaric() {
  ...
  const loadPayments = async () => {
    setLoading(true)
    try {
      const data = await paymentsAPI.getAll({ payment_type: 'supplier' })
      setPayments(data)
    } catch (err: any) {
  ...
  const loadSuppliers = async () => {
    try {
      ...
      showToast(err.response?.data?.message || 'Təchizatçılar yüklənə bilmədi', 'error')
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!paymentForm.supplier_id) {
      showToast('Təchizatçı seçilməlidir', 'error')
      return
    }
    ...
    setLoading(true)
    try {
      await paymentsAPI.createSupplierPayment({
      ...
      loadPayments()
      loadBalance()
      loadSuppliers() // Balans yenilənməsi üçün
    } catch (err: any) {
  ...
  const selectedSupplier = suppliers.find(s => s.id === paymentForm.supplier_id)

  return (
    <Layout>
      <div style={{ padding: '1rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <h1 style={{ fontSize: '1.5rem', margin: 0 }}>Kassa Mexaric (Təchizatçılara Ödəniş)</h1>
          <button
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', background: 'white' }}>
              <thead>
                <tr style={{ background: '#f5f5f5' }}>
                  ...
                  <th style={{ padding: '0.75rem', textAlign: 'left', borderBottom: '2px solid #ddd' }}>Təchizatçı</th>
                ...
                {payments.length === 0 ? (
                  ...
                  payments.map((payment) => (
                    <tr key={payment.id} style={{ borderBottom: '1px solid #eee' }}>
                      ...
                      <td style={{ padding: '0.75rem', textAlign: 'right', fontWeight: 'bold', color: '#d32f2f' }}>
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  <select
                    ...
                    onChange={(e) => setPaymentForm({ ...paymentForm, supplier_id: e.target.value ? parseInt(e.target.value) : null })}
                    style={{
                      width: '100%',
                      padding: '0.5rem',
                  ...
                  >
                    ...
                    {suppliers.map((supplier) => (
                      <option key={supplier.id} value={supplier.id}>
                        {supplier.name} {supplier.balance !== null && `(Borc: ${Number(supplier.balance).toFixed(2)} AZN)`}
                      </option>
                    ))}
                  </select>
                </div>

                {selectedSupplier && selectedSupplier.balance !== null && (
                  <div style={{
                    marginBottom: '1rem',
                    padding: '0.75rem',
                    background: '#fff3cd',
                    ...
                    <strong>Cari borc:</strong> {Number(selectedSupplier.balance).toFixed(2)} AZN
                  </div>
                )}
